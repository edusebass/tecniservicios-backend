...
const editarProducto = async (req, res) => {
    try {
        const { id } = req.params;
        const { marca, labrado, caracteristicas, alto, ancho, rin, costo, linkimg, cantidad, pvp } = req.body;

        // Validar los datos antes de la actualización

        const productoActualizado = await Producto.findOneAndUpdate(
            { _id: id },
            { marca, labrado, caracteristicas, alto, ancho, rin, costo, cantidad, pvp, linkimg },
            { new: true }
        );
